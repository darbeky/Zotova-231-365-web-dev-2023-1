...
function evaluate(str) {
    // your code here
    str=String(str)
    let example=compile(str);
    let exampleArr=[];
    for (let i=0;i<example.length;i++){
        if (isNumeric(example[i])===true){
            exampleArr.push(+example[i])// если текущий знак это число то мы преобразуем его тип в nomber и добавляем в массив

        }
        else{
            exampleArr.push(example[i])//если текущий знак это операнд то мы просто добавляем его в массив
        }
    }
    while (exampleArr.length!==1){
        for (let i=0;i<exampleArr.length;i++){
            if (isOperation(exampleArr[i])===true){
                if (exampleArr[i]=='+'){
                    exampleArr[i]=(exampleArr[i-2]+exampleArr[i-1]);
                    exampleArr.splice(i-2,2)

                }
                else if (exampleArr[i]=='-'){
                    exampleArr[i]=(exampleArr[i-2]-exampleArr[i-1]);
                    exampleArr.splice(i-2,2)
                }
                else if (exampleArr[i]=='*'){
...
function clickHandler(event) {
    ...
    if (target.classList.contains('digit') || target.classList.contains('operation') || target.classList.contains('bracket')) {
        ...
        let result = +evaluate(screen.textContent);
        // screen.textContent = String(result.toFixed(2));


        resultTwo=String(result)
        if (String(result).includes('.')){
            resultTwo=resultTwo.split('.')[0]
        }
        if (result==resultTwo){
